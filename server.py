...
# === GARMIN SNAPSHOT PIPELINE (consolidado en snapshot.py) ===
# `_collect_day_snapshot` y sus 18 parches viven ahora en snapshot.py.
# Este bloque conserva las piezas dependientes del runtime del servidor:
# rutas de depuración de sueño y los wrappers multi-día de Garmin.
...
# === GARMIN GET_SLEEP_DATA MULTI-DAY GLUE ===
# La implementación multi-día y su depuración viven en sleep.py.
try:
...
# === SNAPSHOT PIPELINE (consolidado en snapshot.py) ===
import snapshot
...
snapshot.configure(
    ...
    _get_api=_get_api,
    _optional_call_first=_optional_call_first,
    _collect_extra_raw=_collect_extra_raw,
    _resting_hr=_resting_hr,
    _extract_vo2=_extract_vo2,
    _sleep_metrics=_sleep_metrics,
    _stress_metrics=_stress_metrics,
    _body_battery_metrics=_body_battery_metrics,
    _hrv_metrics=_hrv_metrics,
    _training_readiness_metrics=_training_readiness_metrics,
    _normalize_activity=_normalize_activity,
    _select_training_readiness_entry=_select_training_readiness_entry,
    _attach_frontend_view_to_snapshot=_attach_frontend_view_to_snapshot,
    _get_hrv_debug_last=lambda: _HRV_SELECTION_DEBUG_LAST,
)
_collect_day_snapshot = snapshot._collect_day_snapshot
